...
function updateTitle() {
  ...
  document.cookie = `title=${new_title};`;
...
function setup() {
  ...
  parts.forEach(part => {
    if (part.indexOf("target=") != -1) {
      let target_string = part.substring(
        part.indexOf("target=") + "target=".length
      );
      console.log(target_string);
      target = new Date(target_string);
    } else if (part.indexOf("title=") != -1) {
      let target_string = part.substring(
        part.indexOf("title=") + "title=".length
      );
      console.log(target_string);
      title.textContent = target_string;
    }
  });
